...
async function scrapeGamePage(gameUrl: string): Promise<StreamLink[]> {
  ...
  // Collect all stream IDs and their labels
  const streamEntries: { id: string; label: string }[] = [];

  // Get the main iframe to extract the embed URL pattern and first stream ID
  const mainIframeSrc = $('#cx-iframe').attr('src') || $('iframe.embed-responsive-item').attr('src');

  // Get stream IDs from changeStream buttons
  ...
  // If no buttons found but we have a main iframe, extract the ID from it
...
export const buffstreams: StreamSource = {
  name: 'Buffstreams',
