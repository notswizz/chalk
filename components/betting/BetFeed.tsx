...
export function BetFeed({ gameId, gameTitle, gameOver, teams }: { gameId: string; gameTitle: string; gameOver?: boolean; teams?: string[] }) {
  ...
  const displayBets = tab === 'open' ? bets.filter((b) => b.status === 'open') : myBets;
  ...
  return (
    <div className="mt-8">
      ...
      {loading ? (
        ...
        <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 252, scrollbarWidth: 'thin', scrollbarColor: 'var(--dust-medium) transparent' }}>
          {displayBets.map((bet) => (
